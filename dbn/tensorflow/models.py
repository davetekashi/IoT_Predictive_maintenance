...
def bias_variable(value, shape, dtype=tf.float32):
    initial = tf.compat.v1.constant(value, shape=shape, dtype=dtype)
    return tf.compat.v1.Variable(initial)
...
class BinaryRBM(BaseBinaryRBM, BaseTensorFlowModel):
    ...
    def _initialize_weights(self, weights):
        if weights:
            ...
            if self.activation_function == 'sigmoid':
                ...
                self.W = weight_variable(tf.compat.v1.random_normal, [self.n_hidden_units, self.n_visible_units], stddev)
                self.c = weight_variable(tf.compat.v1.random_normal, [self.n_hidden_units], stddev)
                self.b = weight_variable(tf.compat.v1.random_normal, [self.n_visible_units], stddev)
    ...
    def _build_model(self, weights=None):
        ...
        self.compute_hidden_units_op = self._activation_function_class(
            tf.transpose(tf.matmul(self.W, tf.compat.v1.transpose(self.visible_units_placeholder))) + self.c)
        ...
        self.random_uniform_values = tf.compat.v1.Variable(tf.compat.v1.random_uniform([self.batch_size, self.n_hidden_units]))
        sample_hidden_units_op = tf.compat.v1.to_float(self.random_uniform_values < self.compute_hidden_units_op)
        ...
        positive_gradient_op = tf.compat.v1.matmul(tf.compat.v1.expand_dims(sample_hidden_units_op, 2),  # [N, U, 1]
                                         tf.compat.v1.expand_dims(self.visible_units_placeholder, 1))  # [N, 1, V]
        ...
        for t in range(self.contrastive_divergence_iter):
            compute_visible_units_op = self._activation_function_class(
                tf.compat.v1.matmul(sample_hidden_units_gibbs_step_op, self.W) + self.b)
            compute_hidden_units_gibbs_step_op = self._activation_function_class(
                tf.compat.v1.transpose(tf.compat.v1.matmul(self.W, tf.compat.v1.transpose(compute_visible_units_op))) + self.c)
            random_uniform_values = tf.compat.v1.Variable(tf.compat.v1.random_uniform([self.batch_size, self.n_hidden_units]))
            sample_hidden_units_gibbs_step_op = tf.compat.v1.to_float(random_uniform_values < compute_hidden_units_gibbs_step_op)
            self.random_variables.append(random_uniform_values)

        negative_gradient_op = tf.compat.v1.matmul(tf.expand_dims(sample_hidden_units_gibbs_step_op, 2),  # [N, U, 1]
                                         tf.compat.v1.expand_dims(compute_visible_units_op, 1))  # [N, 1, V]

        compute_delta_W = tf.compat.v1.reduce_mean(positive_gradient_op - negative_gradient_op, 0)
        compute_delta_b = tf.compat.v1.reduce_mean(self.visible_units_placeholder - compute_visible_units_op, 0)
        compute_delta_c = tf.compat.v1.reduce_mean(sample_hidden_units_op - sample_hidden_units_gibbs_step_op, 0)

        self.update_W = tf.compat.v1.assign_add(self.W, self.learning_rate * compute_delta_W)
...
class TensorFlowAbstractSupervisedDBN(BaseAbstractSupervisedDBN, BaseTensorFlowModel):
    ...
    def _initialize_weights(self, weights):
        if weights:
            for attr_name, value in weights.items():
                self.__setattr__(attr_name, tf.compat.v1.Variable(value))
        else:
            if self.unsupervised_dbn.activation_function == 'sigmoid':
                stddev = 1.0 / np.sqrt(self.input_units)
                self.W = weight_variable(tf.compat.v1.random_normal, [self.input_units, self.num_classes], stddev)
                self.b = weight_variable(tf.compat.v1.random_normal, [self.num_classes], stddev)
                self._activation_function_class = tf.compat.v1.nn.sigmoid
            elif self.unsupervised_dbn.activation_function == 'relu':
                stddev = 0.1 / np.sqrt(self.input_units)
                self.W = weight_variable(tf.compat.v1.truncated_normal, [self.input_units, self.num_classes], stddev)
                self.b = bias_variable(stddev, [self.num_classes])
                self._activation_function_class = tf.compat.v1.nn.relu
            else:
                raise ValueError("Invalid activation function.")
    ...
    def _build_model(self, weights=None):
        ...
        visible_units_placeholder_drop = tf.compat.v1.nn.dropout(self.visible_units_placeholder, keep_prob)
        ...
        for rbm in self.unsupervised_dbn.rbm_layers:
            rbm_activation = rbm._activation_function_class(
                tf.compat.v1.transpose(tf.compat.v1.matmul(rbm.W, tf.compat.v1.transpose(rbm_activation))) + rbm.c)
            keep_prob = tf.compat.v1.placeholder(tf.float32)
            self.keep_prob_placeholders.append(keep_prob)
            rbm_activation = tf.compat.v1.nn.dropout(rbm_activation, keep_prob)
        ...
        # operations
        self.y = tf.compat.v1.matmul(self.transform_op, self.W) + self.b
        self.y_ = tf.compat.v1.placeholder(tf.float32, shape=[None, self.num_classes])
...
class SupervisedDBNClassification(TensorFlowAbstractSupervisedDBN, ClassifierMixin):
    ...
    def _build_model(self, weights=None):
        ...
        self.output = tf.compat.v1.nn.softmax(self.y)
        self.cost_function = tf.compat.v1.reduce_mean(tf.compat.v1.nn.softmax_cross_entropy_with_logits_v2(logits=self.y, labels=tf.compat.v1.stop_gradient(self.y_)))
